...
class SetPetSafeZoneCenterUseCaseTest : BaseArchTest() {

    private val petRepository: PetRepository = mock()
    private val setPetSafeZoneCenterUseCase =
        SetPetSafeZoneCenterUseCase(
            petRepository
        )
    ...
    fun `should set safe zone center for pet`() = runBlocking {
        ...
        whenever(petRepository.setPetSafeZoneCenter("Fluffy", center)).thenReturn(expected)
        Truth.assertThat(setPetSafeZoneCenterUseCase("Fluffy", center)).isEqualTo(expected)
    }
