...
import com.ulusoyapps.pettrackerapp.domain.interactors.pet.GetAllPetsUseCase
...
class GetAllPetsUseCaseTest : BaseArchTest() {

    private val petRepository: PetRepository = mock()
    private val getAllPetsUseCase = GetAllPetsUseCase(petRepository)
    ...
    @Test
    fun `should get all pets`() = runBlocking {
        whenever(petRepository.getAllPets()).thenReturn(flow)

        getAllPetsUseCase().collectIndexed { index, value ->
            if (index == 0) {
                Truth.assertThat(value).isEqualTo(Err(MockError))
            }
            if (index == 1) {
                Truth.assertThat(value.get()).isEqualTo(listOf(pet))
            }
        }
    }
}