...
class SavePetUseCaseTest : BaseArchTest() {
    private val petRepository: PetRepository = mock()
    private val savePetUseCase =
        SavePetUseCase(
            petRepository
        )
    ...
    fun `should save pet`() = runBlocking {
        ...
        whenever(petRepository.savePet(pet)).thenReturn(expected)
        Truth.assertThat(savePetUseCase(pet)).isEqualTo(expected)
    }
