...
package com.ulusoyapps.pettrackerapp.datasource.petandtracker.datasource.local
...
class PetAndTrackerLocalDataSource
@Inject constructor(
    private val datasource: PetAndTrackerCache
): PetAndTrackerDataSource {
    override suspend fun getPetAndTracker(petName: String): Flow<Result<PetAndTracker, DomainMessage>> {
        return datasource.getPetAndTracker(petName)
    }

    override suspend fun getAllPetAndTrackers(): Flow<Result<List<PetAndTracker>, DomainMessage>> {
        return datasource.getAllPetAndTrackers()
    }
}